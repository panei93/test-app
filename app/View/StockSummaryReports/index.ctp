...
<script>
	...
	function Download_Report(){
		document.forms[0].action = "<?php echo $this->webroot; ?>StockSummaryReports/Download_Summary_Rpt";
						...
						<table class="table table-bordered" style="margin-top:10px;width:100%;" id="tbl_manager_comment">
							...
							<tbody>
								<?php 
									...
									$destination_name = h($row['stocks']['destination_name']);
									$business_code = h($row['stocks']['layer_code']);
									$business_name = h($row['stocks']['name_jp']);
										
						           
									$numberofdays = h($row['stocks']['numbers_day']);
									$jp_amount = h($row['stocks']['amount']);
								...
								<tr>
									...
									<td style="width: 5%; word-wrap: break-word;" class="prev_commt">
										<?php echo "$add_cmt1 $add_cmt2 $add_cmt3 $rev_cmt4" ?>
									</td>
								</tr>
								<?php }?>	
							</tbody>								
						...
						<table class="table table-bordered" style="margin-top:10px;width:100%;" id="summary_rpt_2">
							...
							<tbody>
								...
								foreach($search_result_two as $rowTwo){
									...
									$destination_name = h($rowTwo['stocks']['destination_name']);
									$business_code = h($rowTwo['stocks']['layer_code']);
									$business_name = h($rowTwo['stocks']['name_jp']);
									$schedule_date = h($rowTwo['0']['schedule_date']);
									
						            $numberofdays = h($rowTwo['stocks']['numbers_day']);
									$amountofMoney = h($rowTwo['amountofMoney']);
									//$comment = $rowTwo['saps']['preview_comment'];
									$jp_amount = h($rowTwo['stocks']['amount']);
