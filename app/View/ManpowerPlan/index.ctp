...
<script type="text/javascript">
	...
	function excelDownload(){
		...
		document.forms['0'].action = "<?php echo $this->webroot; ?>ManpowerPlan/ManpowerExcelDownload";
...
<div class="container register_container">
	...
	<?php else: ?>
		<div class="form-group row" style="text-align:right;">
			...
			<div class="col-md-12">
				<label id="btn_browse" class = "upload-div">Upload File
			...
			if(pos_name_list){
				...
				buttons: {                    
					ok: {                 
						...
						action: function(){ 
							loadingPic();               
							document.forms[0].action = "<?php echo $this->webroot; ?>ManpowerPlan/saveManpower";
							document.forms[0].method = "POST";
		...
		if ($('#'+idName).length) {
			...
			if ($('#'+idName).hasClass('amount_input') || $('#'+idName).hasClass('total_input')) {
				...
				if(idName.indexOf("amount") !== -1){
					...
					$('#'+idName).html(formatNumber(calVal.toFixed(2),2));
	...
	function CalculateTotal(idName,difference,columnName) {
		...
		if ($('#'+idName).length) {
			...
			if(idName.indexOf("amount") !== -1 || (idName.indexOf("manualamt") !== -1 && idName.indexOf("-0-") == -1 )){
				...
				$('#'+totalID).html(formatNumber(totalCalVal.toFixed(2),2));
   			...
   			$.confirm({
				...
				buttons: {
					ok: {
						...
						action:function(){
							loadingPic(); 									
							document.forms[0].action = "<?php echo $this->webroot; ?>ManpowerPlan/saveUploadFile";
							document.forms[0].method = "POST";
							document.forms[0].submit();  
