...
<style>
	...
	.trading-table tr td.colorFill, .trading-table tr td input.colorFill, .trading-table tr td input[disabled]:not(.clone_copy, .clone_remove) {
...
<input type="hidden" id="logi_list" name="logi_list" value="">
<input type="hidden" name="model_name" value="<?php echo $modelName; ?>">
<input type="hidden" name="sum_model_name" value="<?php echo $sumModelName; ?>">
		...
		<div class="row">
			<div class="col-sm-9">
				<div class="form-group row">
					<div class="col-md-8 col-sm-12">
						...
						<div class="col-md-8">
							<input type="text" class="form-control" id="budget_term" name="budget_term" value="<?php echo $term_name; ?>" readonly/>
				...
				<div class="form-group row">
					<div class="col-md-8 col-sm-12">
						<label for="object" class="col-md-4 col-form-label">
							<?php echo $layer_type_name." ".__("コード");?>
						</label>
						<div class="col-md-8">
							<input type="text" class="form-control" id="layer_code" name="layer_code" value="<?php echo $layer_code.'/'.$layer_code_name; ?>" readonly/>
							<input type="hidden" id="layer" name="layer" value="<?php echo $layer_code; ?>">
							<input type="hidden" id="layer_name" name="layer_name" value="<?php echo $layer_code_name; ?>">
						</div>
					</div>					
				</div>
				<div class="form-group row">
				...
				<input type="hidden" class="form-control" id="filling_date" name="filling_date" value="<?php echo $trade_data['trade_filling_date']; ?>" />
			...
			<?php else: ?>
				<div class="col-sm-12 col-md-12 text-right adjust">
					<div class="row">
						<label id="btn_browse" class="btn">Upload File
				...
				<!-- <div class="col-sm-12 col-md-12 text-right adjust"> -->
					<!-- <div class="row">
						<label id="btn_browse" class="btn btn-success btn_sumisho one"><?php echo __("一括アップロード"); ?>
				...
				<div class="col-md-12 align-right">
 					<input type="button" name="btn_edit_logistic" id="btn_edit_logistic" class="btn btn-success" value="<?php echo __("取引順番調整"); ?>">
				...
				<?php if (!empty($trade_data)): ?>
					<div id="table-wpr">
						...
						<table class="trading-table" id="totalTable">
							...
							<tbody>
								<?php foreach ($trade_data['total'] as $acc_name => $acc_datas): ?>
									<tr class="double-border">
									<td colspan="4" class="account_name"> <?php echo $acc_name; ?> </td>
									<?php $result_month = $start_month; ?>
									<?php foreach ($acc_datas['total']['amount'] as $month_name => $each_month):
										$acc_id = (!empty($acc_datas['total']['acc_id'])) ? $acc_datas['total']['acc_id'] : 3;
										
										$negclass = ($each_month < 0) ? 'negative' : '';
 										?>
										<td class="total_field <?php echo $negclass.$bgclass; ?>" id="sum_<?php echo $month_name."_".$acc_id; ?>"><?php echo number_format($each_month,1); ?></td>
										<!-- hidden summary sub amt -->
										<td style="display: none;" class="total_field <?php echo $negclass.$bgclass; ?>" id="hid_sum_<?php echo $month_name.'_'.$acc_id; ?>">
											<?php echo number_format($each_month,3); ?>
										</td>
										
									<?php endforeach ?>
									</tr>
									<?php foreach ($acc_datas['data'] as $sub_name => $value):
										$code = $value['code']; ?>
										<tr>
											<td class="gap"></td>
											<td colspan="3" class="account_name"><?php echo $sub_name; ?></td>
											<?php $result_month = $start_month; ?>
											<?php foreach ($value['amount'] as $month => $each_month):
												$bgclass = (!empty($result_disabled[$month])) ? ' colorFill ' : '';
												$negclass = ($each_month < 0) ? 'negative' : '';
												?>
												<td class="total_field <?php echo $negclass.$bgclass ?>" id="sum_<?php echo $month."_".$acc_id."_".$code; ?>"><?php echo number_format($each_month,1); ?></td>
												<!-- hidden summary acc amt -->
												<td style="display: none;" class="total_field <?php echo $negclass.$bgclass; ?>" id="hid_sum_<?php echo $month.'_'.$acc_id.'_'.$code; ?>">
													<?php echo number_format($each_month,3); ?>
												</td>
											<?php endforeach ?>
					...
					foreach ($trade_data['record'] as $index_name => $trades):
						$tradecnt++;
						$logi_disabled = $del_disabled = $trade_data['disabled_logi'][$index_name];
						?>
						
						<table class="trading-table sub-tbl" id="table_<?php echo $tradecnt ?>">
							<div class="error text_error" id="text_error_<?php echo $tradecnt ?>"></div>
							<thead>
								<tr>
									<th class="w-20 noBorder"></th>
									<th colspan="2" class="w-210 noBorder"></th>
									<th class="noBorder" colspan="16" height="20px">
										<input type="button" data="<?php echo($index_name) ?>" class="btn btn-danger btn-sm pull-right delete_table_btn" name="trade[<?php echo($tradecnt) ?>][delete_table_btn]" value="<?php echo __('テーブル削除') ?>" <?php echo $del_disabled; ?>/>
									</th>
								...
								<tr>
									...
									<span style="font-weight: bold;"><?php echo __('取引') ?></span>		
										...
										name=""  <?php echo($logi_disabled); ?>>
											<option value="" selected>---- Select Logistic Index ----</option>
											<?php foreach($logistic_data as $name) : ?>
												<option value="<?php echo $name;?>" <?php if(trim($index_name) == trim($name)){?> selected <?php } ?> ><?php echo $name; ?>
												</option>
											<?php endforeach ?>
										</select>
										
										<!-- hidden logistic -->
										<input type="hidden" id="hid_<?php echo('logistic_index_'.$tradecnt); ?>"  class="logistic_index form-control" name="trade[<?php echo($tradecnt) ?>][<?php echo('logistic_index'); ?>]" value="<?php echo($index_name); ?>">
										<span class="warning_text"><?php echo __('⇒取引名をタブより選択して下さい。新規追加する際は財務経理部へご連絡下さい。') ?></span>
									</td>
								</tr>
							</thead>
							<tbody>
								<?php foreach ($trades['trading'] as $acc_name => $acc_datas): ?>
									...
									foreach ($acc_datas['total']['amount'] as $month_name => $each_month):
										$negclass = ($each_month < 0) ? 'negative' : ''; ?>

										<td class="total_field <?php echo $negclass ?>" id="sum_<?php echo $tradecnt."_".$month_name."_".$acc_id; ?>"><?php echo number_format($each_month, 1); ?></td>
										<!-- hidden sub acc amt -->
										<td style="display: none;" class="total_field <?php echo $negclass ?>" id="hid_sum_<?php echo $tradecnt."_".$month_name."_".$acc_id; ?>">
											<?php echo number_format($each_month, 3); ?>
										</td>
									<?php endforeach ?>
									</tr>
									<?php $cnt=0; foreach ($acc_datas['data'] as $sub_acc_name => $value): ?>
										<!-- Account eg.(売上高（仕切) -->
										
										<?php 
											$acc_code = $value['code'];
											if (strpos($sub_acc_name, '社内受払手数料') !== false):
											$acc_name_explode = explode('/#/', $sub_acc_name);
											$sub_acc_name = $acc_name_explode[0];
										...
										<tr class="input_row" id="<?php echo("tr_".$cnt); ?>">
											...
											<?php } ?>
											<td class="account_name" style="border-right: none;padding: 0px 1px !important;letter-spacing: -1px;"><?php echo $sub_acc_name; ?></td>
											<td style="border-left: none;">
												
												<input class="input home_payment_fee<?php echo($bgclass); ?>" id="<?php echo('fee_name_'.$tradecnt.'_tr_'.$cnt); ?>" name="" value="<?php echo($user_input); ?>" <?php echo $user_disable; ?>>
												<!-- hidden home_payment_fee -->
												<input type="hidden" class="form-control home_payment_fee" id="hid_<?php echo('fee_name_'.$tradecnt.'_tr_'.$cnt); ?>" name="trade[<?php echo($tradecnt) ?>][data][<?php echo($acc_id) ?>][<?php echo($acc_code) ?>][<?php echo('tr_'.$cnt); ?>][user_input]" value="<?php echo($user_input); ?>">
											</td>
											<td style="border-left: none;">
												<select class="select_box form-control select_destination" name="" id="<?php echo('select'.$tradecnt.'_tr_'.$cnt); ?>" <?php echo $dis; ?>>
												...
												<!-- hidden destination -->
												<input type="hidden" class="form-control hid select_destination" name="trade[<?php echo($tradecnt) ?>][data][<?php echo($acc_id) ?>][<?php echo($acc_code) ?>][<?php echo('tr_'.$cnt); ?>][destination]" id="hid_<?php echo('select'.$tradecnt.'_tr_'.$cnt); ?>" value="<?php echo($des); ?>">
											</td>
											<?php $result_month = $start_month;
												 ?>
											<?php foreach ($amount as $month => $each_amount):
												...
												<td class="total_field <?php echo $negclass ?> " id="<?php echo "sum_".$tradecnt."_".$month."_".$acc_id."_".$acc_code."_tr_".$cnt; ?>"><?php echo number_format($each_amount,1); ?>
												</td>
												<!-- hidden acc total amt -->
												<td style="display: none;" class="total_field <?php echo $negclass ?> " id="hid_<?php echo "sum_".$tradecnt."_".$month."_".$acc_id."_".$acc_code."_tr_".$cnt; ?>">
											...
											<?php else: 
												...
												$disabled = $result_disabled[$month];
												if(!empty($result_disabled[$month])) {
													$sameID = ' rec_'.$tradecnt."_".$month."_".$acc_id."_".$acc_code;
													$bgclass = $color_fill = ' colorFill ';
													$hid_each_amount = 0;
												}
											...
											?>
												...
												<td class="<?php echo $color_fill ?>">
													<input id="<?php echo 'rec_'.$tradecnt."_".$month."_".$acc_id."_".$acc_code."_tr_".$cnt; ?>" type="" name="" max="9" class="amount_input <?php echo $bgclass.$negclass.$sameID; ?>" value="<?php echo number_format($each_amount,1); ?>" <?php echo $disabled ?>>
													<!-- hidden acc amt -->
													<input id="hid_<?php echo 'rec_'.$tradecnt."_".$month."_".$acc_id."_".$acc_code."_tr_".$cnt; ?>" type="hidden" name="trade[<?php echo($tradecnt) ?>][data][<?php echo($acc_id) ?>][<?php echo($acc_code) ?>][<?php echo('tr_'.$cnt); ?>][<?php echo($month) ?>]" max="9" class="amount_input <?php echo $bgclass.$negclass.$sameID; ?>" value="<?php echo number_format($hid_each_amount,3); ?>">
												</td>
											<?php endif;
										endforeach ?>
										</tr>
										...
										<?php else: ?>
										<?php 
										if(!empty($value['acc_id'])) $acc_id = $value['acc_id'];
										$acc_code = $value['code'];
										foreach ($value['amount'] as $des => $amount) { ?>
										<tr class="input_row">
											<td class="gap">
											</td>
											<td colspan="3" class="account_name"><?php echo $sub_acc_name; ?>
											</td>
											...
											<?php if (($month == 'first_half') || ($month == 'second_half') || ($month == 'whole_total')): ?>
												<td class="total_field <?php echo $negclass ?>" id="sum_<?php echo $tradecnt."_".$month."_".$acc_id."_".$acc_code; ?>"><?php echo number_format($each_amount,1); ?>
												</td>
												<!-- hidden total amt -->
												<td style="display: none;" class="total_field <?php echo $negclass ?>" id="hid_sum_<?php echo $tradecnt."_".$month."_".$acc_id."_".$acc_code; ?>">
													<?php echo number_format($each_amount,3); ?>
												</td>
											<?php else: 
													...
													$hid_each_amount = $each_amount;
													$disabled = $result_disabled[$month];
													$name = 'trade['.$tradecnt.'][data]['.$acc_id.']['.$acc_code.']['.$month.']';
													if(!empty($result_disabled[$month])) {
														$sameID = ' rec_'.$tradecnt."_".$month."_".$acc_id."_".$acc_code;
														$bgclass = $color_fill = ' colorFill ';
														$hid_each_amount = 0;
													...
													?>
												<td class="<?php echo $color_fill ?>">
													<input id="rec_<?php echo $tradecnt."_".$month."_".$acc_id."_".$acc_code; ?>" type="" name="" max="9" class="amount_input <?php echo $bgclass.$negclass.$sameID; ?>" value="<?php echo number_format($each_amount,1); ?>" <?php echo $disabled ?>>
													<!-- hidden acc amt -->
													<input id="hid_rec_<?php echo $tradecnt."_".$month."_".$acc_id."_".$acc_code; ?>" type="hidden" name="<?php echo $name; ?>" max="9" class="amount_input <?php echo $bgclass.$negclass.$sameID; ?>" value="<?php echo number_format($hid_each_amount,3); ?>">
												</td>
											<?php endif ?>
											<?php endforeach ?>
										</tr>
								...
								<tr class="double-border input_row">
									...
									<td style="border-left: none;">
										...
										?>
										<input id="<?php echo('kpi_unit_input'.$tradecnt); ?>" class="kpi_unit_input" type="" name="" value="<?php echo $kpi_unit[0]; ?>">
										<!-- hidden kpi unit -->
										<input id="hid_<?php echo('kpi_unit_input'.$tradecnt); ?>" class="kpi_unit_input" type="hidden" name="trade[<?php echo($tradecnt) ?>][kpi_unit]" value="<?php echo $kpi_unit[0]; ?>">
									</td>
									</td>
									<?php foreach ($table_months as $month): ?>
										...
										<?php elseif($month == 'second_half'): ?>
											...
											<td class="color_fill"><input id="rec_<?php echo $tradecnt."_".$month."_0_0000000000"; ?>" type="" name="" max="9" class="kpi_amount_input <?php echo $negclass ?>" value="<?php echo number_format($kpi_amt,1); ?>">
											<!-- hidden kpi amt -->
											<input id="hid_rec_<?php echo $tradecnt."_".$month."_0_0000000000"; ?>" type="hidden" name="trade[<?php echo($tradecnt) ?>][data][0][0000000000][<?php echo($month) ?>]" max="9" class="kpi_amount_input <?php echo $negclass ?>" value="<?php echo number_format($kpi_amt,3); ?>">
											</td>
										<?php endif ?>
									<?php endforeach ?>
								</tr>
...
<div class="modal fade" id="adjust_order_popup" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
	<div class="modal-dialog modal-dialog-scrollable" role="document" style="width: 80%">
		<div class="modal-content">
			<div class="modal-header" style="background-color:#8dc98d">
				<h5 class="modal-title" id="exampleModalScrollableTitle"><?php echo __("取引表示順の調整"); ?></h5>
			...
			<div class="modal-body" style="max-height: 500px; overflow-y: auto;">
				...
				<div style="margin-bottom: 100px">	
					<button type="button" name="editLogisticButton" id="editLogisticButton" class="btn btn-success" style="float: right;"><?php echo __('Save Order');?> </button>
				</div>
			</div>
		</div>
	</div>
</div>	
<!-- Logistic setupPopUpBox End  -->
<?php
	echo $this->Form->end();
?>
<?php echo $this->Html->script('jquery.tablednd.js'); ?>
...
<script type="text/javascript">
	...
	$("#btn_browse").css({"color": "#fff"});
	if(approved_BA != '' || page == 'Disabled'){

		$('input[type="text"], .amount_input, .delete_table_btn, #btn_add_new_tab, .kpi_unit_input, .kpi_amount_input, .select_destination, .clone_copy, .clone_remove, .home_payment_fee, #btn_edit_logistic').each(function () {
			$(this).prop('disabled', true);
		})

		$('.logistic_index, #btn_save,.trading_upload, .uploadbulkfile').prop('disabled', true);
		$("#btn_browse, .uploadbulkfile").css({"opacity": 0.5, "cursor": "not-allowed",
							  "background-color": "#D5EADD", "color": "#fff"});
	...
	$(document).ready(function(){
						...
						buttons: {
					        ok: {
								...
								btnClass: 'btn-info',
					          	action:function(){
									loadingPic(); 									
									document.forms[0].action = "<?php echo $this->webroot; ?>BrmTradingPlan/UploadFile";
		...
		$(document).on('keyup','.select2-search__field ', function (event) {
			var val = $.trim($('.select2-search__field').val());
			var $table = $('.select2-search__field').attr("aria-controls").substring(8,32).replace(/[^0-9]/gi, '');
			var keycode = (event.keyCode ? event.keyCode : event.which);
			if(checkSpecialChar(val[0]) || keycode == '13') {
				$("#text_error_"+$table).empty();
				...
				$("#text_error_"+$table).html(errMsg(commonMsg.JSE019,['<?php echo __("最初の文字")?>'])).show();
				return false;
			} 
			else {
				return true;
		...
		$('.logistic_index').on('select2:select', function (e) {
			$("#text_error_"+this.id.replace(/[^0-9]/gi, '')).empty();
		});
		$('.logistic_index').on('select2:close', function (e) {
			$("#text_error_"+this.id.replace(/[^0-9]/gi, '')).empty();
		});	
		...
		$('#btn_add_new_tab').click(function(){ //Clone the table
			...
			var div1 = $("#text_error_"+prevnum).clone(); //Clone the element. Assigned to the JS variable div1
			div1.attr("id", "text_error_"+num); //Modify the necessary properties using $(element).attr()
			div1.empty();
			$('#table-wpr').append(div1);
			...
			$('#table_'+num+' tr td').each(function(index, value) {
				...
				$(this).find('.logistic_index').each(function(){
					...
					var select2_id = $(this).attr('data-select2-id');
					...
					$(this).attr('disabled', false); 
					 ...
					 if(logID.indexOf('hid') == -1) {
						if ($('#'+logID).hasClass("select2-hidden-accessible")) {
							// Select2 has been initialized
							$('#'+logID).removeAttr('data-select2-id');
							$('#'+logID).removeClass('select2-hidden-accessible');
							$('#table_'+num+' .select2').attr("style", "display: none !important;");
						}
					}
				});
		...
		$("#btn_save").click(function(){
			...
			document.getElementById("logi_list").value   = null;
			...
			if(chk) { 
				document.getElementById("logi_list").value = logarr.join(',');
				$.confirm({           
					...
					buttons: {                    
						ok: {                 
							...
							action: function(){ 
								loadingPic();                   
								document.forms[0].action = "<?php echo $this->webroot; ?>BrmTradingPlan/saveTradingData";
								document.forms[0].method = "POST";
								document.forms[0].submit();    
		...
		$('.table-group').on('change','.amount_input', function(){
			...
			if(decimalOnly.test($(this).val().replace(/,/g, "")) == false) {
				...
				//console.log($hidId+" = "+current+" = "+prev+" = "+diff);
		...
		$('.table-group').on('click','.delete_table_btn', function() {
			...
			$('#text_error_'+delTableId[1]).remove();
			$.confirm({           
					...
					ok: {                 
						...
						action: function(){ 
							...
							$('.sub-tbl').each(function(a,b) {
								...
								$("#text_error_"+prevnum).attr('id',"text_error_"+num);
		...
		$('.table-group').on('mouseenter','.logistic_index', function() {
			...
			result.sort(function(a, b) {
				return logi_dd_list.toString().indexOf(a) - logi_dd_list.toString().indexOf(b);
			}); 
			...
			$('#'+this.id).select2().on('select2:open', function(e){
		...
		$('#btn_bulk_excel_download').click(function(){
			document.getElementById("error").innerHTML   = "";
			document.getElementById("success").innerHTML = "";
			document.forms[0].action = "<?php echo $this->webroot; ?>BrmTradingPlan/downloadBulkExcelDownload";
			document.forms[0].method = "POST";
			document.forms[0].submit();
			return true;
		});
				...
				$.confirm({                     
					...
					buttons: {                                      
						ok: {                                   
							...
							action: function(){        
									loadingPic();
									document.forms[0].action = "<?php echo $this->webroot; ?>BrmTradingPlan/saveBulkExcelFile";
									document.forms[0].method = "POST";
									document.forms[0].submit();										 
									return true;    
							}                        
		...
		/* Edit logistic data to adjust */
		$("#btn_edit_logistic").click(function(e){
			var year    = $('#year').val();
			var ba_data = $('#layer').val();
			var ba_data = ba_data.substring( ba_data.indexOf( "/" ) +1, ba_data.length );
			var layername = $('#layer_name').val();
			$.ajax({
				type : "POST",
				url : "<?php echo $this->webroot; ?>BrmTradingPlan/getLogisticData",
				data : {year : year},
				dataType : 'json',
				success : function(logistics_data){

					$('.acc_setup .sortable').html('');
					var table_html = '';
					var len = logistics_data.length;
					data_arr = [];	
					...
					var myObj = logistics_data;

					var ba_name = "";
				
					for(var i in logistics_data){	
						layer_name = layername;
						var tmp_arr = {};
						var id           = logistics_data[i].BrmLogistic.id;
						var layer_code      = logistics_data[i].BrmLogistic.layer_code;
						var year         = logistics_data[i].BrmLogistic.target_year;
						var index_name   = logistics_data[i].BrmLogistic.index_name;
						var logi_order   = logistics_data[i].BrmLogistic.logistic_order;

						//Prepare Data
						tmp_arr['id']           = id;
						tmp_arr['year']         = year;
						tmp_arr['layer_code']      = layer_code;
						tmp_arr['layer_name']      = layer_name;
						tmp_arr['index_name']   = index_name;
						tmp_arr['order']        = logi_order;

						//Prepare table rows
						table_html += "<tr id="+count+">";
						table_html += "<td>"+count+"</td>";
						table_html += "<td>"+ year +"</td>";	
						table_html += "<td>"+layer_code+"</td>";
						table_html += "<td>"+ layer_name +"</td>";	
						table_html += "<td>"+ index_name +"</td>";	
						// table_html += "<td>"+ related_data[i]tLogisticSetupModel.order+"</td>";
		...
		/* Edit settLogistic pop up link*/
		$("#editLogisticButton").click(function(e){
			$.confirm({
			title: "<?php echo __('保存確認'); ?>",
			icon: 'fas fa-exclamation-circle',
			type: 'green',
			...
			ok: {
				...
				action: function(){
					$.ajax({
					type : "POST",
					url : "<?php echo $this->webroot; ?>BrmTradingPlan/EditLogisticSetup",
