				...
				<?php  
					foreach($prepare_layer as $pkey => $pvalue) {
						foreach($pvalue as $key => $value) {
							if(in_array($value['layer_code'], $readLayer[$value['type_order']])){
								$showTable = true;
							}
						}
					}
					...
					<table class="tbl-bu-analysis bold-border table1" id="tbl_bu">
						<thead class="check_period_table bold-border-btm" style="position: sticky;top:0;">
							<tr class="bold-border-top bold-border-lft bold-border-rgt">
								<th colspan="2" class="bRight layer" style="min-width: 80px" ><?php echo __("部署"); ?></th>
							...
							<?php //pr($prepare_layer);
								...
								?>
									<tr class="row-level" type_order="<?php echo $value['type_order'];?>">
										...
										?>
										<td colspan="<?php echo $colspan_cnt; ?>" class = "bb-none layer layer-cell <?php echo($name_field).' '.$cLevel; ?>" ><?php echo "$name"; ?></td>
										<td class="item-name <?= $cancel_flag == 1  ? 'bg-lg-red' : ($completed==1 ? '' : ($falg==2 || $completed==2 ? 'bg-lg-green' : '')) ?>">
											<?php
												if($flag == 2){
											?>
												<?php if($completed != 1) echo __('入力完了'); ?>
											<?php
												}
												if($showBtn == true && $completed != 2){
											?>
												<button type="button" class="btn-complete" onClick='completeClick("<?php echo $layer_code ?>",<?php echo $parent_id ?>, "<?php echo 'Summary';?>")'>
													<?php echo __('入力確定');?>
												</button>
											<?php 
												}
												// echo "flag = $flag, show = $showBtn, complete = $completed";
												if($cancel_flag == 1 && $showBtn == false){
											?>
												<?php echo __('入力解除'); ?>
											<?php } ?>
										</td>
										<td class="item-name <?= $cancel_flag == 1 ? 'bg-lg-red' : ($completed==1 ? '' : ($falg==2 || $completed==2 ? 'bg-lg-green' : '')) ?>"><?php echo $value['user_name'];?></td>
										<td class="bRight item-name <?= $cancel_flag == 1 ? 'bg-lg-red' : ($completed==1 ? '' : ($falg==2 || $completed==2 ? 'bg-lg-green' : '')) ?>"><?php echo $value['updated_date'];?></td>
									</tr>
									<?php
											}
										}
