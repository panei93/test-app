...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('.disabled-datepicker').css('pointer-events', 'none');
		// $('#addComtCmt2 ').on('keydown', function(event) {
		// 	// Get the pressed key code
		// 	var keyCode = event.keyCode || event.which;
		// 	// console.log(keyCode);return;
		// 	// Check if the pressed key is a number
		// 	if (keyCode >= 48 && keyCode <= 57 || keyCode === 189 || keyCode === 8) {
		// 	return; // Allow numbers,minus & backspace
		// 	}
			
		// 	event.preventDefault();
    	// 	return false;
		// });
		$("td").on('input','#addComtCmt2', function() {$(this).val('');});	
	...
	function searchSR(){
		...
		form.action = "<?php echo $this->webroot; ?>StockAddComments/searchSaleRepresentative";	
		form.method = "GET";
		form.submit();
    		return true;	
	...
	function Excel_download_btn(){
		...
		document.forms[0].action = "<?php echo $this->webroot; ?>StockAddComments/Download_Add_Comment";
    ...
    function getMail(func, noSend = false) {
        ...
        var page = 'StockAddComments';
        var data = [];
        
        $("#mailSubj").val('');
        $("#mailBody").val('');

        var submitForm = (func == 'save')? 'SaveStockAddComments' : (

        					( func == 'request')? 'RequestStockAddComments' : (

        						( func == 'approve')? 'ApproveStockAddComments' : (

        							(func == 'approve_cancel')? 'ApproveCancelStockAddComments' :(

        								(func == 'reject')? 'RejectStockAddComments': 

        									(func == 'review')? 'ReviewStockAddComments': ''
        ...
        $.ajax({
            type:'post',
            url: "<?php echo $this->webroot; ?>StockAddComments/getMailLists",
                ...
                if(mailSend == 1) { 
	                ...
	                if(data.mailType == 1) {
						...
						if(data.bcc != undefined) {
                  		...
                  		loadingPics();
	          			document.forms['add_comment_form'].action = "<?php echo $this->webroot; ?>StockAddComments/"+submitForm;
						...
						$(".body").html(data.body);
						$("#add_comment_form").attr('action', '<?php echo $this->webroot; ?>StockAddComments/'+submitForm);
...
<div class = 'container register_container'>
	...
	<div class ="row line">
		...
		<form id="AddCmtSearch">
			<!-- edit by khin hnin myo for logistics_no search -->
			<div class = "col-md-6">
				<div class = "form-group">
					<label class="col-md-4 control-label"><?php echo __("入庫Index No."); ?></label>
					<div class="col-md-6">
						<input class ='form-control register' style="margin-bottom: 7px; width: 80% !important" type = "textbox" id='receipt ' name="receipt" value = '<?php if(!empty('$searchlogistics')){echo h($searchlogistics);} ?>'>   
					</div> 
				</div>
			</div>
			<!-- end -->
				...
				<div class="table-responsive tbl-wrapper">
					<table class="table-bordered" id="tbl_AddComment" style="min-width: 1900px;">
						<thead class="check_period_table">
							<tr >
								<!-- <th rowspan="2" <?php if($show_btn['approve']){ ?>width="2%" <?php }else if($show_btn['save']){?> width = "6%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "6%" <?php } ?> class="table-middle" style="vertical-align : middle; text-align:center;" ><?php echo __("部署"); ?> -->
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="2%" <?php }else if($show_btn['save']){?> width = "3%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review'] ){?> width = "6%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("保管場所"); ?>
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="2%" <?php }else if($show_btn['save']){?> width = "3%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "6%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("品目コード"); ?>
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="1%" <?php }else if($show_btn['save']){?> width = "6%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "6%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("品目テキスト"); ?>
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="2%" <?php }else if($show_btn['save']){?> width = "5%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "5%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("品目名2"); ?>
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="2%" <?php }else if($show_btn['save']){?> width = "5%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "5%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:5rem;"></div><?php echo __("単位"); ?>
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="2%" <?php }else if($show_btn['save']){?> width = "5%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "5%" <?php } ?>class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:9rem;"></div><?php echo __("入庫日"); ?>
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="2%" <?php }else if($show_btn['save']){?> width = "3%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "4%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:5rem;"></div><?php echo __("滞留日数"); ?>
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="2%" <?php }else if($show_btn['save']){?> width = "6%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "5%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("入庫Index No."); ?>
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="2%" <?php }else if($show_btn['save']){?> width = "5%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "5%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:7rem;"></div><?php echo __("数量"); ?>
									
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "10%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "8%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("金額"); ?>
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "10%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "8%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:5rem;"></div><?php echo __("不完全品 有・無"); ?>
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "10%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "8%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:5rem;"></div><?php echo __("売り繋ぎ 済・未済"); ?>
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "10%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "8%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:5rem;"></div><?php echo __("契約 有・無"); ?>
								</th>
								<th rowspan="2" <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "10%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "8%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("経理コメント"); ?>
								</th>
								<!-- <th rowspan="2" <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "10%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "8%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><?php echo __("解決"); ?>
								</th> -->
								<th colspan="4" <?php if($show_btn['approve']){ ?>width="14%" <?php }else if($show_btn['save']){?> width = "21%" <?php } else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "24%" <?php } ?>class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("担当者コメント入力欄"); ?>
									<!-- default width=20% -->
								</th>
								<?php if(!$show_btn['save']){ ?>
							...
							<tr> 
								<th <?php if($show_btn['approve']){ ?>width="1.5%" <?php }else if($show_btn['save']){?> width = "4%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "3%" <?php } ?> class="table-middle " style="vertical-align : middle;text-align:center;" ><div style="width:3rem;"></div><?php echo __("確認完了"); ?>
								...
								<th <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "10%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "9%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("滞留理由"); ?></th>
								<th <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "9%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "8%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("決済日"); ?></th>
								<th <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "9%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){?> width = "8%" <?php } ?> class="table-middle" style="vertical-align : middle;text-align:center;" ><div style="width:10rem;"></div><?php echo __("備考"); ?></th>

								<?php if(!$show_btn['save']){ ?>

									<th <?php if($show_btn['approve']){ ?>width="1%" <?php }else if($show_btn['save']){?> width = "3%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){ ?> width = "3%" <?php } ?> class="table-middle show_level_6_5" style="vertical-align : middle;text-align:center;" ><div style="width:3rem;"></div><?php echo __("確認済"); ?>
										<?php if($show_btn['request']){ ?>
										<input type="checkbox" name="chk_acc_mgr_confirm" id="chk_acc_mgr_confirm">
										<?php } ?>
									</th>
									<th <?php if($show_btn['approve']){ ?>width="4%" <?php }else if($show_btn['save']){?> width = "7%" <?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel'] || $show_btn['review']){ ?> width = "8%" <?php } ?> class="table-middle show_level_6_5" style="vertical-align : middle; text-align:center;" ><div style="width:10rem;"></div><?php echo __("コメント入力欄"); ?></th>
								<?php }if($show_btn['review'] || count(array_filter($show_btn)) < 1) {?>
									<th <?php if($show_btn['approve']){ ?>width="2%" <?php }else{?> width = "3%" <?php } ?> class="table-middle show_level_4" style="vertical-align : middle; text-align:center;" ><div style="width:3rem;"></div><?php echo __("確認欄");?>
										<br>
									...
									<th <?php if($show_btn['approve']){ ?>width="4%" <?php }else{?> width = "7%" <?php } ?> class="table-middle show_level_4" style="vertical-align : middle; text-align:center;" ><div style="width:10rem;"></div><?php echo __("コメント"); ?></th>
								<?php } ?>
							</tr>						
						</thead>
						<tbody>
							...
							if(!empty($page)) foreach($page as $row){
								...
								$rno++;
								$stock_id 			= h($row['Stock']['id']);			
								$layerCode 			= h($row['Stock']['layer_code']);
								$destination_name	= h($row['Stock']['destination_name']);
								$item_code		 	= h($row['Stock']['item_code']);
								$item_name	 		= h($row['Stock']['item_name']);
								$item_name_2	 	= h($row['Stock']['item_name_2']);
								$unit				= h($row['Stock']['unit']);
								$numbers_day		= h($row['Stock']['numbers_day']);
								$receipt_index_no	= h($row['Stock']['receipt_index_no']);
								$quantity			= h($row['Stock']['quantity']);
								$amount 			= number_format( $row[0]['amount']);
								$is_error 			= h($row['Stock']['is_error']);
								$is_sold 			= h($row['Stock']['is_sold']);
								$is_contract 		= h($row['Stock']['is_contract']);
								$preview_comment 	= h($row['Stock']['preview_comment']);
								$solution 			= h($row['Stock']['solution']);
								$registration_date 	= h($row[0]['stock_registration_date']);
								if($registration_date == "0000-00-00" || $registration_date == ''){
									$registration_date = "";
								}
								$Recorded_date = h($row['Stock']['recorded_date']);
								if($Recorded_date == "0000-00-00" || $Recorded_date == ''){
									$Recorded_date = "";
								}

								$yen_amount = number_format($row['0']['jp_amount']);
								
								$sale_representative = h($row['Stock']['sale_representative']);
								$flag = h($row['Stock']['flag']);
								$busi_incharge_reason = h($row['StockBusiInchargeComment']['reason']);
								...
								$busi_incharge_settlement_date = $row['StockBusiInchargeComment']['settlement_date'];
								...
								$busi_incharge_remark = trim(h($row['StockBusiInchargeComment']['remark']));
								$bac_comment = trim(h($row['StockBusiAdminComment']['comment']));
								$aic_comment = trim(h($row['StockAccInchargeComment']['comment']));
								$BICflag  = $row['StockBusiInchargeComment']['stock_id'];
								$BACFlag  = $row['StockBusiAdminComment']['stock_id'];	
								$AICFlag  = $row['StockAccInchargeComment']['stock_id'];
								$prevCmt  = trim(h($row['Stock']['preview_comment']));
								...
								if($show_btn['save']){
									...
									?>
										<tr class = 'flag_chk_color'>
											<td style="display: none;">
												<input type="hidden" name="flag" class="flag" id="<?php echo 'flag'.$stock_id; ?>" value="<?php echo $flag; ?>">
											</td>
											<!-- <td style=" word-wrap: break-word;" class="account_name"><?php echo $layerCode; ?> -->
												<input type="hidden" name="account_code" class="account_code" value="<?php echo $layerCode; ?>">
											</td>
											<td style="word-wrap: break-word;" class="dest_name"><?php echo $destination_name; ?>
											</td>
											<td style="word-wrap: break-word;" class="item_code"><?php echo $item_code; ?>
											</td>
											<td style="word-wrap: break-word;" class="item_name"><?php echo $item_name; ?>
											</td>
											<td style="word-wrap: break-word;" class="itme_name_2"><?php echo $item_name_2; ?>
											</td>
											<td style="word-wrap: break-word;" class="unit"><?php echo $unit; ?>
											</td>
											<td style="word-wrap: break-word;" class="rg_date"><?php echo $registration_date; ?>
											</td>
											<td style="word-wrap: break-word;" class="no_of_day"><?php echo $numbers_day; ?>
											</td>
											<td style="word-wrap: break-word;" class="rp_no"><?php echo  $receipt_index_no; ?>
											</td>
											<td style="word-wrap: break-word;" class="qty"><?php echo $quantity; ?>
											</td>
											<td style="word-wrap: break-word;" class="yen_amt"><?php echo $amount; ?>
											</td>
											<td style="word-wrap: break-word;" class="is_error"><?php echo $is_error; ?>
											</td>
											<td style="word-wrap: break-word;" class="is_sold"><?php echo $is_sold; ?>
											</td>
											<td style="word-wrap: break-word;" class="is_contract"><?php echo $is_contract; ?>
											</td>
											<td style="word-wrap: break-word; width: 240px;" class="preview_comment">
												<textarea name="preview_comment" id="preview_comment" class="form-control preview-comment" cols="" rows="" readonly><?php echo $preview_comment; ?></textarea>
											</td>
											<!-- <td style="word-wrap: break-word;" class="solution"><?php echo $solution; ?>
											</td> -->
											...
											<td style="word-wrap: break-word; margin-top: 16px;">
												<div class="input-group date datepicker disabled-datepicker" data-provide="" data-date-format="yyyy-mm-dd">
													<input type ="text" name="addComtCmt2" id = "addComtCmt2" value = "<?php echo trim($busi_incharge_settlement_date); ?>" class="form-control" autocomplete="off" disabled="">
									...
									<?php }else{   ?>
										<tr>
											...
											<td style="word-wrap: break-word;" class="preview_comment">
												<textarea name="preview_comment" id="preview_comment" class="form-control preview-comment" cols="" rows="" readonly><?php echo $preview_comment; ?></textarea>
											</td>
											<!-- <td style="word-wrap: break-word;" class="solution"><?php echo $solution; ?>
											</td> -->
											...
											<?php if(!empty($BICflag)=='1' && $flag == '2'){ ?>
												<td style="word-wrap: break-word;text-align: center;">
													<input type="checkbox" class = "chk_addComt1 chk1" name = "chk_addComt1" value = "<?php echo $stock_id;?>" class="test_line">
												</td>
											<?php }else{ ?>
												<td style="word-wrap: break-word; text-align: center;">
													<input type="checkbox" class = "chk_addComt1 chk1" name = "chk_addComt1" value = "<?php echo $stock_id;?>" class="test_line" <?php if($flag >= 3){ ?> checked="" <?php }else{} ?> >
												</td>

												<?php if($flag == '3'){ ?>
														<input type="hidden" class = "hiddenFlag3" name = "hiddenFlag3" value = "<?php echo $stock_id;?>">
													<?php }?>
											...
											<td style="word-wrap: break-word; width: 8%;">
												...
												<select class="form-control retention_reason" id="addComtCmt1" name="addComtCmt1" style="width:10rem;">
											...
											<td style="word-wrap: break-word; margin-top: 16px;">
												<div class="input-group date datepicker" data-provide="datepicker" data-date-format="yyyy-mm-dd" style="width:10rem;">
													<input type ="text" name="addComtCmt2" id = "addComtCmt2" autocomplete="off" value = "<?php echo trim($busi_incharge_settlement_date); ?>" class="form-control settle_date">
													<span class="input-group-addon">
														<span class="glyphicon glyphicon-calendar"></span>
													</span>
												</div>
											...
											<td style="word-wrap: break-word;">
												...
												<textarea rows="3" cols="2" class="form-control add_comment_textarea incharge_remark" name="addComtCmt3" id = "addComtCmt3" style="width:10rem;"><?php echo $busi_incharge_remark; ?></textarea>
								...
								<?php }else if($show_btn['review'] || $show_btn['approve_cancel']){ 
										...
										?>
											<tr class = 'flag_chk_color'>
												...
												<td style="word-wrap: break-word; margin-top: 16px;">
													<div class="input-group date datepicker disabled-datepicker" data-provide="" data-date-format="yyyy-mm-dd">
														<input type ="text" name="addComtCmt2" id = "addComtCmt2" autocomplete="off" value = "<?php echo trim($busi_incharge_settlement_date); ?>" class="form-control" disabled="">
														<span class="input-group-addon">
															<span class="glyphicon glyphicon-calendar"></span>
														</span>
													</div>
									...
									<?php }else{  ?>
											...
											<tr>
												...
												<td style="word-wrap: break-word; text-align: center;">
													<input type="checkbox" class = "chk_addComt1" name = "chk_addComt1" value = "<?php echo $stock_id;?>" class="test_line" checked="" disabled="disabled">
												</td>
												</td>
												<td style="word-wrap: break-word;">
												...
												<td style="word-wrap: break-word; text-align: center;" class="show_level_6_5">
													<input type="checkbox" class = "chk_addComt2" name = "chk_addComt2" value = "<?php echo $stock_id;?>" checked="" disabled=''>
												...
												<td style="word-wrap: break-word;" class="show_level_4">
													<?php if($flag =='5'){ ?>
													<input type="checkbox" value = "<?php echo $stock_id;?>" name="chk_addComt3" class="chk_addComt3 chk3">
												<?php }else if($flag =='6'){ ?>
													<input type="checkbox" value = "<?php echo $stock_id;?>" checked= "" name="chk_addComt3" class="chk_addComt3 chk3">
												<?php } ?>
								...
								<?php }else if($show_btn['request'] || $show_btn['approve'] || $show_btn['approve_cancel']){
									...
									<?php }else{  ?>
											...
											<tr>
												<td style="display: none;">
													<input type="block" name="flag" class="flag" id="<?php echo 'flag'.$stock_id; ?>" value="<?php echo $flag; ?>">
												</td>
												<!-- <td style=" word-wrap: break-word;" class="account_name"><?php echo $layerCode; ?> -->
													<input type="hidden" name="account_code" class="account_code" value="<?php echo $layerCode; ?>">
												</td>
												...
												<td style="word-wrap: break-word; text-align: center;">
													<input type="checkbox" class = "chk_addComt1" name = "chk_addComt1" value = "<?php echo $stock_id;?>" checked="" disabled="disabled">
												</td>
												</td>
												<td style="word-wrap: break-word;">
												...
												<td style="word-wrap: break-word;text-align: center;" class="show_level_6_5">
													<?php if(!empty($BACFlag)== '1' && $flag == '3'){ ?>
														<input type="checkbox" class = "chk_addComt2 chk2" name = "chk_addComt2" value = "<?php echo $stock_id;?>">
													<?php }else{ ?>
														<input type="checkbox" class = "chk_addComt2 chk2" name = "chk_addComt2" value = "<?php echo $stock_id;?>" <?php if($flag >= 4){ ?> checked="" <?php }else{} ?>>
													<?php } ?>
														<?php if($flag == '4'){ ?>
															<input type="hidden" class = "hiddenFlag4" name = "hiddenFlag4" value = "<?php echo $stock_id;?>">
														<?php }?>
												</td>
												<td style="word-wrap: break-word;" class="show_level_6_5">
