...
;(function ( $, window, document, undefined ) {
  ...
  ComboTree.prototype.init = function () {
    ...
    this._elemArrowBtn = $('<div id="' + this.comboTreeId + 'ArrowBtn" class="comboTreeArrowBtn" type="button"><span class="comboTreeArrowBtnImg"><i class="fa fa-caret-down"></i></span></div>');
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    ...
    if (isThereSubs)
      itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse || collapse ? '<span class="mdi mdi-chevron-right-circle-outline fa fa-plus-circle"></span>' : '<span class="mdi mdi-chevron-down-circle-outline fa fa-minus-circle"></span>') + '</span>'; // itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '+' : '&minus;') + '</span>';

    if (this.options.isMultiple){
      if (isThereSubs){
        itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeParent comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? '<input type="checkbox" />' : '') + sourceItem.title + '</span>';
      }else{
  ...
  ComboTree.prototype.toggleSelectionTree = function (item, direction) {
    ...
    if (direction === undefined){
      if ($(subMenu).is(':visible'))
        $(item).children('span.comboTreeParentPlus').html('<span class="mdi mdi-chevron-right-circle-outline fa fa-plus-circle"></span>'); //$(item).children('span.comboTreeParentPlus').html("+");
      else
        $(item).children('span.comboTreeParentPlus').html('<span class="mdi mdi-chevron-down-circle-outline fa fa-minus-circle"></span>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");

      $(subMenu).slideToggle(50);
    }
    else if (direction == 1 && !$(subMenu).is(':visible')){
      $(item).children('span.comboTreeParentPlus').html('<span class="mdi mdi-chevron-down-circle-outline fa fa-plus-circle"></span>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");
      $(subMenu).slideDown(50);
    }
    else if (direction == -1){
      if ($(subMenu).is(':visible')){
        $(item).children('span.comboTreeParentPlus').html('<span class="mdi mdi-chevron-right-circle-outline fa fa-minus-circle"></span>'); //$(item).children('span.comboTreeParentPlus').html("+");
        $(subMenu).slideUp(50);
      }
      else {
  ...
  ComboTree.prototype.refreshInputVal = function () {
    ...
    if (this.options.isMultiple) {
      for (var i=0; i<this._selectedItems.length; i++){
        if(this._selectedItems[i].className.indexOf("comboTreeParent") < 0){
    ...
    if(baCount > 2)
      this._elemInput.val(baCount + " Selected");
    else
  ...
  ComboTree.prototype.setSelection = function (selectionIdList) {
    ...
    this._selectedItems = this._selectedItems.map(object => {
