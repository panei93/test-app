...
/**
 * BrmForecastSummary Model
 *
 * @property BrmTerm $BrmTerm
 */
class BrmForecastSummary extends AppModel {
 ...
 */
	public $useTable = 'brm_forecast_summary';
 ...
 */
	...
	public function updateSummaryData($ba_code,$term_id,$target_year) {
		...
		$del_param = array();
		$del_sql  = "";
		$del_sql .= "DELETE from brm_forecast_summary";
		...
		$this->query("INSERT INTO `brm_forecast_summary` (brm_term_id, target_year, hlayer_code, hlayer_name, dlayer_code, dlayer_name, layer_name_jp, layer_code, logistic_index_no, brm_account_id_2, brm_account_name_jp_2, brm_account_id_5, brm_account_name_jp_5, brm_account_id_6, brm_account_name_jp_6, month_1_amt, month_2_amt, month_3_amt, month_4_amt, month_5_amt, month_6_amt, first_half, month_7_amt, month_8_amt, month_9_amt, month_10_amt, month_11_amt, month_12_amt, second_half, whole_total)
				...
				FROM brm_expected
				LEFT JOIN `layers` ON (
					`layers`.`layer_code` = `brm_expected`.`layer_code` AND
					`layers`.`flag` = 1 AND brm_expected.flag=1
				)
				...
				LEFT JOIN `layers` as `layersDEPT` ON (
					...
					`subacc2`.`account_code` = `brm_expected`.`account_code` AND
					`subacc2`.`group_code` = '02' AND brm_expected.flag=1
				)LEFT JOIN 
					...
					`subacc5`.`account_code` = `brm_expected`.`account_code` AND
					`subacc5`.`group_code` = '05' AND brm_expected.flag=1
				)LEFT JOIN 
					...
					`subacc6`.`account_code` = `brm_expected`.`account_code` AND
					`subacc6`.`group_code` = '06') AND brm_expected.flag=1
				WHERE brm_expected.flag=1 AND brm_expected.account_code!='0000000000' AND brm_expected.layer_code = '".$ba_code."' AND brm_term_id = '".$term_id."' AND target_year = '".$target_year."'
				GROUP BY 
