...
class SapProgressReportsController extends AppController
{
    public $uses = array('User', 'SapAccManagerApprove', 'Sap', 'SapBusiManagerApprove', 'LayerType');
    ...
    public function beforeFilter()
    {
        ...
        parent::checkSapUrlSession('progress');
    ...
    public function index()
    {
        ...
        $sap_data = $this->Sap->getBACodeFlag($period);
        
        $complete_ba = $this->Sap->getCompleteBACode($period);

        $count = count($sap_data);
        if (empty($sap_data)) {
            $no_data = parent::getErrorMsg("SE001");
        } else {
            $no_data = "";
            $result = $this->__prepareApprovedData($sap_data, $complete_ba);
        }
    ...
    public function __prepareApprovedData($sap_data, $complete_ba)
    {
        # prepare data
        ...
        if ($count_complete > 0) {
            for ($j=0; $j<$count_complete; $j++) {
                $complete_ba_arr[] = $complete_ba[$j]['saps']['layer_code'];
            }
        }
        
        for ($i=0; $i<$count; $i++) {
            $done = '';
            $layer_code = $sap_data[$i]['tmp']['layer_code'];
            $max_flag = $sap_data[$i]['tmp']['max_flag'];
            $min_flag = $sap_data[$i]['tmp']['min_flag'];
            $name_jp = $sap_data[$i]['tmp']['name_jp'];

            $parent_id = $sap_data[$i]['tmp']['parent_id'];
            $busi_approve_date = $sap_data[$i]['busi_approve']['approve_date'];
            $acc_approve_date = $sap_data[$i]['acc_approve']['approve_date'];
    ...
    public function progress_pdf()
    {
        ...
        if ($this->request->is('post')) {
            $period = $this->Session->read('SapSelections_PERIOD_DATE');
            $get_data = $this->Sap->getBACodeFlag($period);
            $complete_ba = $this->Sap->getCompleteBACode($period);
            $data = $this->__prepareApprovedData($get_data, $complete_ba);
            $this->set(compact('data', 'period'));
        } else {
            $this->redirect(array('controller'=>'SapProgressReports', 'action'=>'index'));
        }
    }
}
